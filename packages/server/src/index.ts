...
program
  .name("naip-server")
  .option("--root <path>", "扫描入口目录（会递归扫描 .pdf）", process.env.NAIP_ROOT || "")
  .option("--db <path>", "SQLite 索引文件路径", process.env.NAIP_DB || path.resolve(".data/index.sqlite"))
  ...
  .option("--serve-web", "生产模式：同时静态托管 web/dist", Boolean(process.env.NAIP_SERVE_WEB || false))
  .option("--web-dist <path>", "web 的 dist 目录（配合 --serve-web）", process.env.NAIP_WEB_DIST || "");
